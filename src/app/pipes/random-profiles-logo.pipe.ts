...
@Pipe({
  name: 'randomProfilesLogo'
})
export class RandomProfilesLogoPipe implements PipeTransform {

  constructor(private _sanitizer: DomSanitizer) {}
    
  transform(value: any): SafeHtml
  {
    let first=value.candidateFirstName
    let second=value.candidateLastName
    let data=first[0]+second[0]

    let termsApply = '';
    let background = this.generateRgb(value.profileID);
